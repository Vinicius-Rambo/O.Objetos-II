...
public class DaoVeiculo {
    ...
    public ArrayList<Veiculo> buscarTodos(){
        ...
        try {
            this.conectar();
            ResultSet rs = st.executeQuery("SELECT * FROM tbveiculos ORDER BY modelo"); //ResultSet: Armazena os vindo do banco de dados, em formato de tabela.
    ...
    public Veiculo consultar(int cod){
        ...
        try {
            this.conectar();
            ResultSet rs = st.executeQuery("SELECT * FROM tbveiculos WHERE codigo = " + cod + ";"); //ResultSet: Armazena os vindo do banco de dados, em formato de tabela.

            while(rs.next()){ //Vai um por um até o proximo ser vazio, não dá null pointer
                v = new Veiculo(); //Dentro do objeto veiculo, ele seta os valores seguinda as tabelas do banco de dados.
